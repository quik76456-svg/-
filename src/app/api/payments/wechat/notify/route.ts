...
import { wechatProvider } from "@/server/providers/payments/wechat";

export async function POST(req: Request) {
  try {
    const { orderId, providerTradeNo, amountCny } = await wechatProvider.verifyAndParseNotify(req);
    ...
    await db.$transaction(async (tx) => {
      ...
      if (current.status !== "FULFILLED") {
        const paid = await markOrderPaid(orderId, "WECHAT", providerTradeNo, amountCny, tx);
